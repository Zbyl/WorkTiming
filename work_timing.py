...
def load_data_from_event_log(date_start, *, print_query_only=False):
...
def load_data_from_csv(file_name):
    ...
    Copy data manually from the Event Viewer, from Windows Logs > Create Custom View > XML, and input query like this:
    @note Regenerate query, as it contains dates.
    <QueryList>
      <Query Id="0" Path="Security">
        <Select Path="Security">*[System[(EventID=4800 or EventID=4801 or EventID=7002 or EventID=1074) and TimeCreated[timediff(@SystemTime) &lt;= 3153103146]]]</Select>
        <Select Path="System">*[System[(EventID=4800 or EventID=4801 or EventID=7002 or EventID=1074) and TimeCreated[timediff(@SystemTime) &lt;= 3153103146]]]</Select>
      </Query>
    </QueryList>
    Select all event, and copy them to a TSV file (right click > Copy > Copy Table). (@note Events don't need to be sorted by date.)
    ...
    event_data = [EventData(begin=isStartEvent(event[3]), time=datetime.datetime.strptime(event[1], time_format)) for event in filtered_raw_data]

    return event_data


def load_data_from_log(file_name, date_start):
    """
    Returns list of lists of values from login.txt file (generated by login.bat).
    :param file_name:   login.txt file.
    :return: List of EventData tuples.
    """
    ...
    unexpected = [(time, kind) for time, kind in tuples if kind not in ['Logged in', 'Unlocked', 'Locked']]
    if len(unexpected) > 0:
        raise Exception(f'Unexpected events: {unexpected}')

    # Convert dates to datetime.
    time_format = '%Y-%m-%d-%H:%M'
    event_data = [EventData(begin=(kind in ['Logged in', 'Unlocked']), time=datetime.datetime.strptime(time, time_format)) for time, kind in tuples]
    event_data = [event for event in event_data if event.time.astimezone() >= date_start]
...
def main():
    if False:
        events = load_data_from_event_log(global_date_start, print_query_only=True) # @todo Can't make winevt to work any more...
        events = load_data_from_csv('recov-oct0.tsv')
    else:
        events = load_data_from_log('login.txt', global_date_start)
